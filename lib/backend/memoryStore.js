...
class MemoryStore extends EventEmitter {
    ...
    getDataForDashboard() {
        ...
        const hasDataForDashboard = Object.keys(dataForDashboard).length;

        return (hasDataForDashboard) ? dataForDashboard : null;
    }
    ...
    calculateSpeed(droneId) {
        const storeHasDroneId = this.positionData[droneId];

        if (!storeHasDroneId) {
            return null;
        }

        const hasOnlyOnePosition = (this.positionData[droneId].length === 1);
     ...
     */
    getSpeed() {
        const hasSpeedStoreData = Object.keys(this.speedData).length;
        if (!hasSpeedStoreData) {
            return null;
        }
    ...
    flagStationaryDrones(droneId, position) {
        const start = 10000; // In Milliseconds before the current timestamp
        const range = 1000; // In Milliseconds - Range within which to search the last position 
